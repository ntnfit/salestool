    ...
    <div class="content">
        <form action="{{ route('pro.update', ['proid' => $header[0]['ProId']]) }}" method="post">
            ...
            <div class="row">
                <x-adminlte-select label="Type Promotion" label-class="text-lightblue" name="protype" id="protype"
                    ...
                    @foreach ($PromTypes as $promtype)
                        @if($promtype->ProtypeID==$header[0]['PromotionType'])
                        <option value="{{ $promtype->ProtypeID }}" selected>{{ $promtype->ProtypeName }}</option>
                        @else
                ...
                <div class="row col-md-9" style="float:right">
                    ...
                    <x-adminlte-input label="Promotion name" label-class="text-lightblue" name="promotionname" value="{{$header[0]['PromotionName']}}"
                        type="text" fgroup-class="col-md-6" enable-old-support>
                    </x-adminlte-input>
                    @if($header[0]['Special']==1)
                    <x-adminlte-input name="special" label="Special" type="checkbox" label-class="text-lightblue"
                        value="special" fgroup-class="col-xs-3" enable-old-support  checked>
                    </x-adminlte-input>
                    @else
                    <x-adminlte-input name="special" label="Special" type="checkbox" label-class="text-lightblue"
            ...
            <div class="row">
                ...
                <x-adminlte-input label="Quantity" label-class="text-lightblue" name="Quantity" type="number" value="{{$header[0]['Quantity']}}"
                    fgroup-class="col-md-3" enable-old-support>
                </x-adminlte-input>
                <x-adminlte-input label="Amount" label-class="text-lightblue" name="Amount" type="number" value="{{$header[0]['TotalAmount']}}"
                    fgroup-class="col-md-3" enable-old-support>
                </x-adminlte-input>
                <x-adminlte-input label="Dis.Percent" label-class="text-lightblue" name="dispercent" type="number"  value="{{$header[0]['DiscountPercent']}}"
                        ...
                        <tbody>
                            ...
                            <tr class="tr_clone_itemlist">
                                ...
                                <td><select class="uom form-control" name="UomCode[]" >
                                        <option value="" selected></option>
                                        @foreach ($Uoms as $Uom)
                                        @if($listItem['UoMEntry']== $Uom->UomEntry)
                                ...
                                <td>
                                    <select class="form-control" name="BaseUom[]" style="max-width:350px" readonly="true">
                                        <option value="" selected></option>
                                        @foreach ($Uoms as $Uom)
                                        @if($listItem['BaseUoMEntry'] == $Uom->UomEntry)
                                            <option value="{{ $Uom->UomEntry }}" selected>{{ $Uom->UomName }}</option>
                                            @else
                                            <option value="{{ $Uom->UomEntry }}">{{ $Uom->UomName }}</option>
                                        @endif
                            ...
                            <tr class="tr_clone">
                                <td>
                                    <select class="items" name="cus[]" data-placeholder="Select an customer">
                                            ...
                                            <option value="{{ $dt['ProCustCode']}}" selected>
                                                {{ $dt['ProCustCode'] . '--' . $dt['ProCustName'] . '--' .
                                ...
                                <tr class="tr_clone_proitem">
                                    <td>
                                        <select class="proitem" name="proitem[]" data-placeholder="Select an itemcode">
                                            <option></option>
                                            @foreach ($ItemCodes as $ItemCode)
                                            @if($proitem['ProItemCode']==$ItemCode->ItemCode)
                                                <option value="{{ $ItemCode->ItemCode }}" selected>
                                                    {{ $ItemCode->ItemCode . '--' . $ItemCode->ItemName }}</option>
                                                    @else
                                                <option value="{{ $ItemCode->ItemCode }}">
                                    ...
                                    <td>
                                        <select class="form-control" name="prouomcode[]" style="max-width:350px">
                                            <option value="" selected></option>
                                            @foreach ($Uoms as $Uom)
                                            @if($proitem['ProUoMEntry']==$Uom->UomEntry)
                                                <option value="{{ $Uom->UomEntry }}" selected>{{ $Uom->UomName }}</option>
                                            @else
                                            <option value="{{ $Uom->UomEntry }}">{{ $Uom->UomName }}</option>
                                            @endif
                                    ...
                                    <td>
                                            ...
                                            <select class="form-control" name="probaseoum[]" style="max-width:350px" readonly="true">
                                                <option value="" selected></option>
                                                @foreach ($Uoms as $Uom)
                                                    @if($proitem['ProBaseUoMEntry']==$Uom->UomEntry)
                                                    <option value="{{ $Uom->UomEntry }}" selected>{{ $Uom->UomName }}</option>
                                                    @else
                                                    <option value="{{ $Uom->UomEntry }}">{{ $Uom->UomName }}</option>
                                                    @endif
